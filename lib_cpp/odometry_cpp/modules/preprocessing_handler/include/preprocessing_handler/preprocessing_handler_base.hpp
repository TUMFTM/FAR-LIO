...
void filter_points(thrust::device_vector<types::Point<TConfig>>& points, Predicate predicate, const std::string& name,
  std::unordered_map<std::string, std::int64_t>& num_points, std::unordered_map<std::string, double>& preprocess_time,
  ::cuda::stream_ref stream = {})
{
  auto start = std::chrono::high_resolution_clock::now();
  auto end = thrust::remove_if(thrust::cuda::par.on(stream.get()), points.begin(), points.end(), predicate);
...
#else
template <typename TConfig, typename Predicate>
void filter_points(std::vector<types::Point<TConfig>>& points, Predicate predicate, const std::string& name,
  std::unordered_map<std::string, std::int64_t>& num_points, std::unordered_map<std::string, double>& preprocess_time)
{
  auto start = std::chrono::high_resolution_clock::now();
...
class PreprocessingHandler : public OdometryBase<TConfig, types::PreprocessingConfig, types::PreprocessingDebug>
{
public:
  /**
...
protected:
  ...
  PreprocessingHandler(const types::PreprocessingConfig& config, const types::PreprocessingDebug& debug)
...
public:
   ...
   * @param [in] stream             CUDA stream (default: default stream)
   */
  void crop_points(thrust::device_vector<types::Point<TConfig>>& points, ::cuda::stream_ref stream = {})
  {
   ...
   * @param [in] stream             CUDA stream (default: default stream)
   */
  void crop_footprint(thrust::device_vector<types::Point<TConfig>>& frame, ::cuda::stream_ref stream = {})
  {
    ...
    filter_points(
      ...
      [footprint_lat, footprint_lon] __device__(const types::Point<TConfig>& p) {
        ...
        bool within_footprint = (std::abs(p.pos.x()) <= footprint_lon &&  // rear and forward boundary
    ...
    filter_points(
      ...
      [this](const auto& p) {
        ...
        bool within_footprint =
          (std::abs(p.pos.x()) <= this->config_.crop_footprint.front() &&  // rear and forward boundary
