 ...
 */
class GICPParameterizedTest : public ::testing::TestWithParam<TestParams>
{
protected:
  void SetUp() override {}
...
TEST_P(GICPParameterizedTest, GICPRegisterFrameParameterized)
{
  ...
  CHECK_CUDA_ERROR();
  auto map_ = tam::core::state::cuda::VoxelHashMap<tam::core::state::types::CUDA_GICP_EXT>::from_config(pmg_.get(), logger_.get()); // NOLINT
  auto registration_ = tam::core::state::cuda::ICP<tam::core::state::types::CUDA_GICP_EXT>::from_config(pmg_.get(), logger_.get()); // NOLINT
  #else
  auto map_ = tam::core::state::VoxelHashMap<tam::core::state::types::GICP_EXT>::from_config(pmg_.get(), logger_.get()); // NOLINT
  auto registration_ =  tam::core::state::ICP<tam::core::state::types::GICP_EXT>::from_config(pmg_.get(), logger_.get()); // NOLINT
  #endif
  // clang-format on
  ...
  pmg_raw->set_value("map.cov_regularization", params.cov_regularization);
  pmg_raw->set_value("registration.solver_type", params.solver_type);
  pmg_raw->set_value("registration.damping_factor", 1.0e-9);
  ...
  std::normal_distribution<float> quaternion_distribution_init(0.05, 0.001);
  std::normal_distribution<float> translation_distribution_init(1.0, 0.1);
  ...
  init_guess_quaternion.normalize();
