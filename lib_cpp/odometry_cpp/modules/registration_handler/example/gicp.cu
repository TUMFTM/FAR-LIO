...
int main(int argc, char * argv[])
{
  ...
  map_->add_points(map, tam::core::state::types::POINT_NORMAL::MAX_POINTS_PER_VOXEL, 1, tam::core::state::types::CUDA_GICP_EXT::NUM_NEIGHBORS, stream);  // NOLINT
  ...
  std::vector<tam::core::state::types::Point<tam::core::state::types::CUDA_GICP_EXT>> frame = tam::core::state::utils::load_pointcloud_bin<tam::core::state::types::CUDA_GICP_EXT>(argv[2]); // NOLINT
  std::cout << "Loaded " << frame.size() << " points from " << argv[2] << std::endl;
  // Load the initial guess
  Sophus::SE3f init_guess = tam::core::state::utils::load_pose<tam::core::state::types::CUDA_GICP_EXT>(argv[3]);  // NOLINT

  const auto & [frame_registration, frame_map] = tam::core::state::cuda::voxel_doubledownsample<tam::core::state::types::CUDA_GICP_EXT>(frame, map_->get_config().voxel_size);  // NOLINT
