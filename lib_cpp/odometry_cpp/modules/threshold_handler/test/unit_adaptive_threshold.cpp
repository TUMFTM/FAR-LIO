 ...
 */
TEST(AdaptiveThreshold, BuildPmgLogger)
{
  // clang-format off
 ...
 */
TEST(AdaptiveThreshold, BuildConfigDebug)
{
  // clang-format off
  ...
  std::unique_ptr<tam::core::state::ThresholdHandler<tam::core::state::types::ICP_EXT>> threshold_ =  // NOLINT
    tam::core::state::AdaptiveThreshold<tam::core::state::types::ICP_EXT>::from_config(config, debug); // NOLINT
  // clang-format on
  EXPECT_EQ(threshold_->get_config().initial_threshold, 8.0)
