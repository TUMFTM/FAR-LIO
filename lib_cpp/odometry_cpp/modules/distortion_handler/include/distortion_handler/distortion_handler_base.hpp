...
: public OdometryBase<TConfig, types::DistortionConfig, types::DistortionDebug>
{
public:
  /**
  ...
  void set_pose(const types::PoseStamped & pose)
  {
    ...
    auto it = std::lower_bound(
      this->pose_history_.begin(), this->pose_history_.end(), pose,
   ...
   */
#ifdef __CUDACC__
  virtual __host__ void undistort(
  ...
  virtual void undistort(
    std::vector<types::Point<TConfig>> & frame, const std::uint64_t frame_stamp) = 0;
#endif
  /**
...
protected:
  ...
  Sophus::SE3f interpolate_pose(const std::uint64_t stamp) const
  {
    ...
    auto it = std::lower_bound(
      this->pose_history_.begin(), this->pose_history_.end(), stamp,
    ...
    const float t = denom > 0.0
                      ? static_cast<float>(
                          (static_cast<double>(stamp) - static_cast<double>(p0.stamp)) / denom)
                      : 0.0f;
  ...
  bool fit_polynom(
    ...
    Eigen::Matrix<float, 6, types::POLYNOM_DEGREE + 1> & poly_coeff, float & timestamp_offset)
  {
