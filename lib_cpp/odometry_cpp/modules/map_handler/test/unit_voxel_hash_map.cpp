...
TEST(VoxelHashMap, BuildPmgLogger)
{
  ...
  EXPECT_EQ(map_->get_config().voxel_size, 5.0)
    << "Failed to construct VoxeHashMap from param manager and logger";
}
/**
...
TEST(VoxelHashMap, BuildConfigDebug)
{
  ...
  // clang-format on
  EXPECT_EQ(map_->get_config().voxel_size, 1.0)
    << "Failed to construct VoxeHashMap from config and debug object";
}
/**
...
TEST(VoxelHashMap, SearchNeighbor)
{
  ...
  EXPECT_EQ(correspondence.map.pos, Eigen::Vector3f(5.1, 2.1, 3.1))
    << "Wrong closest neighbor found!";
}
/**
...
TEST(VoxelHashMap, Covariances)
{
  ...
  for (const auto & neighbor : neighbors_svd) {
    if ((neighbor.point->pos - Eigen::Vector3f(5.1, 2.1, 3.1)).norm() < 1e-5) {
      EXPECT_TRUE(neighbor.point->cov.isApprox(expected_cov_SVD, 1e-5))
  ...
  for (const auto & neighbor : neighbors_frob) {
    if ((neighbor.point->pos - Eigen::Vector3f(5.1, 2.1, 3.1)).norm() < 1e-5) {
      EXPECT_TRUE(neighbor.point->cov.isApprox(expected_cov_FROBENIUS, 1e-5))
