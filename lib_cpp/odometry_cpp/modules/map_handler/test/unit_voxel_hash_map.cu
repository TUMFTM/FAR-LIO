...
TEST(VoxelHashMapCuda, BuildPmgLogger)
{
  ...
  tam::pmg::ParamReferenceManager::UniquePtr pmg_ =
    std::make_unique<tam::pmg::ParamReferenceManager>();
  tam::tsl::ReferenceLogger::UniquePtr logger_ = std::make_unique<tam::tsl::ReferenceLogger>();
  auto map_ =
    tam::core::state::cuda::VoxelHashMap<tam::core::state::types::POINT_NORMAL>::from_config(
      pmg_.get(), logger_.get());

  tam::pmg::MgmtInterface * pmg_raw = pmg_.get();
  pmg_raw->set_value("map.voxel_size", 5.0);

  EXPECT_EQ(map_->get_config().voxel_size, 5.0)
    << "Failed to construct VoxelHashMapCuda from param manager and logger";
}
/**
...
TEST(VoxelHashMapCuda, SearchNeighbor)
{
  ...
  thrust::device_vector<
    tam::core::state::types::Correspondence<tam::core::state::types::POINT_NORMAL>>
    correspondences(1);
  ...
  std::vector<tam::core::state::types::Correspondence<tam::core::state::types::POINT_NORMAL>>
    correspondences_host(correspondences.size());
  ...
  EXPECT_NEAR(correspondences_host.front().map.pos.x(), 5.1f, 1e-5)
    << "Wrong closest neighbor found (x coordinate)!";
  EXPECT_NEAR(correspondences_host.front().map.pos.y(), 2.1f, 1e-5)
    << "Wrong closest neighbor found (y coordinate)!";
  EXPECT_NEAR(correspondences_host.front().map.pos.z(), 3.1f, 1e-5)
    << "Wrong closest neighbor found (z coordinate)!";
}
/**
...
TEST(VoxelHashMapCuda, SearchNeighbors)
{
  ...
  CHECK_CUDA_ERROR();
  auto map_ = init_map<tam::core::state::types::POINT_NORMAL>();
  std::vector<float> point_1_distances{0.000, 1.136, 1.386, 1.738, 1.349,
                                       1.628, 1.158, 0.943, 1.449, 0.985};

  // Capture neighbors
  auto neighbors = map_->get_neighbors();

  // Copy neighbors to host
  std::vector<tam::core::state::types::Neighbors<tam::core::state::types::POINT_NORMAL>>
    neighbors_host(neighbors.size());
  ...
  std::vector<tam::core::state::types::Point<tam::core::state::types::POINT_NORMAL>> points_host(
    neighbors_host.size());
  for (size_t i = 0; i < neighbors_host.size(); ++i) {
    cudaMemcpy(
      &points_host[i], neighbors_host[i].point,
      sizeof(tam::core::state::types::Point<tam::core::state::types::POINT_NORMAL>),
      cudaMemcpyDeviceToHost);
  }

  EXPECT_EQ(neighbors_host.size(), 10) << "Wrong amount of neighbors found in calculation!";
  EXPECT_EQ(
    neighbors_host.front().num_neighbors, tam::core::state::types::POINT_NORMAL::NUM_NEIGHBORS)
  ...
  for (const auto & neighbor : neighbors_host) {
    if ((points_host[i].pos - Eigen::Vector3f(5.1, 2.1, 3.1)).norm() < 1e-5) {
      ...
      for (size_t i = 0; i < tam::core::state::types::POINT_NORMAL::NUM_NEIGHBORS; ++i) {
        EXPECT_NEAR(distances_computed[i], point_1_distances[i], 1e-3)
          << "Wrong neighbor for point 1!";
      }
      break;
    }
    i++;
...
TEST(VoxelHashMapCuda, Covariances)
{
  ...
  // Copy neighbors to host
  std::vector<tam::core::state::types::Neighbors<tam::core::state::types::POINT_NORMAL>>
    neighbors_svd_host(neighbors_svd.size());
  thrust::copy(neighbors_svd.begin(), neighbors_svd.end(), neighbors_svd_host.begin());
  // Copy the actual points to host, as the neighbors only contain device pointers
  std::vector<tam::core::state::types::Point<tam::core::state::types::POINT_NORMAL>>
    points_svd_host(neighbors_svd_host.size());
  for (size_t i = 0; i < neighbors_svd_host.size(); ++i) {
    cudaMemcpy(
      &points_svd_host[i], neighbors_svd_host[i].point,
      sizeof(tam::core::state::types::Point<tam::core::state::types::POINT_NORMAL>),
      cudaMemcpyDeviceToHost);
  }
  ...
  // Copy neighbors to host
  std::vector<tam::core::state::types::Neighbors<tam::core::state::types::POINT_NORMAL>>
    neighbors_frob_host(neighbors_frob.size());
  thrust::copy(neighbors_frob.begin(), neighbors_frob.end(), neighbors_frob_host.begin());
  // Copy the actual points to host, as the neighbors only contain device pointers
  std::vector<tam::core::state::types::Point<tam::core::state::types::POINT_NORMAL>>
    points_frob_host(neighbors_frob_host.size());
  for (size_t i = 0; i < neighbors_frob_host.size(); ++i) {
    cudaMemcpy(
      &points_frob_host[i], neighbors_frob_host[i].point,
      sizeof(tam::core::state::types::Point<tam::core::state::types::POINT_NORMAL>),
      cudaMemcpyDeviceToHost);
  }
  ...
  // Copy neighbors to host
  std::vector<tam::core::state::types::Neighbors<tam::core::state::types::POINT_NORMAL>>
    neighbors_min_host(neighbors_min.size());
  thrust::copy(neighbors_min.begin(), neighbors_min.end(), neighbors_min_host.begin());
  // Copy the actual points to host, as the neighbors only contain device pointers
  std::vector<tam::core::state::types::Point<tam::core::state::types::POINT_NORMAL>>
    points_min_host(neighbors_min_host.size());
  for (size_t i = 0; i < neighbors_min_host.size(); ++i) {
    cudaMemcpy(
      &points_min_host[i], neighbors_min_host[i].point,
      sizeof(tam::core::state::types::Point<tam::core::state::types::POINT_NORMAL>),
      cudaMemcpyDeviceToHost);
  }

  Eigen::Matrix3f expected_cov_SVD, expected_cov_FROBENIUS, expected_cov_MIN_EIGENVALUE;
  expected_cov_SVD << 0.685181, 0.463466, 0.0243394, 0.463466, 0.317701, -0.0358316, 0.0243394,
    -0.0358316, 0.998118;
  expected_cov_FROBENIUS << 31.1507, 20.4267, 1.02138, 20.4267, 15.6264, -14.2987, 1.02138,
    -14.2987, 286.484;
  expected_cov_MIN_EIGENVALUE << 0.060013, 0.0399809, 0.00199909, 0.0399809, 0.0296282, -0.0279985,
    0.00199909, -0.0279985, 0.560000;
  ...
  for (const auto & neighbor : neighbors_svd_host) {
    if ((points_svd_host[i].pos - Eigen::Vector3f(5.1, 2.1, 3.1)).norm() < 1e-5) {
      EXPECT_TRUE(points_svd_host[i].cov.isApprox(expected_cov_SVD, 1e-5))
        << "Wrong covariance (SVD)!";
      break;
    }
    i++;
  }
  ...
  for (const auto & neighbor : neighbors_frob_host) {
    if ((points_frob_host[i].pos - Eigen::Vector3f(5.1, 2.1, 3.1)).norm() < 1e-5) {
      EXPECT_TRUE(points_frob_host[i].cov.isApprox(expected_cov_FROBENIUS, 1e-3))
        << "Wrong covariance (FROBENIUS)!";
      break;
    }
    i++;
  }
