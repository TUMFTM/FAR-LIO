...
enum class JitterFilterMethod { None = 0, NearestNeighbour = 1, NearestNeighbour80m = 2, AdaptiveNearestNeighbour = 3 };

struct RadarPreprocessingConfig {
  // CROP in spherical coordinates (min/max elevation/azimuth)
  std::vector<double> crop_spherical{-std::numbers::pi, std::numbers::pi,  // azimuth
    -std::numbers::pi / 2, std::numbers::pi / 2};                          // elevation
  // Min/max filtering for RCS
  std::vector<double> rcs_threshold{-std::numeric_limits<double>::infinity(), std::numeric_limits<double>::infinity()};
  // Min/max filtering for SNR
  std::vector<double> snr_threshold{-std::numeric_limits<double>::infinity(), std::numeric_limits<double>::infinity()};
