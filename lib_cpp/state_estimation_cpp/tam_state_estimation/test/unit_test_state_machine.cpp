...
TEST(ExecutionUnitTests, StateMachineInit)
{
  ...
  tam::types::control::Odometry input_odometry_vel{};
  StateEstimation.set_input_linear_velocity(input_odometry_vel, 1);
  StateEstimation.set_input_linear_velocity_status(tam::types::ErrorLvl::OK, 1);
...
TEST(ExecutionUnitTests, StateMachineVelWarn)
{
  /**
   * expected output        |    OK   |
   * ----------------------------------
  ...
  // Linear Velocity Timeout
  StateEstimation.set_linear_velocity_timeout(1);

  // compare the state estimation output
  EXPECT_EQ(StateEstimation.get_status(), tam::types::ErrorLvl::WARN);
}
...
TEST(ExecutionUnitTests, StateMachineOneBackupIMUError)
{
  ...
  tam::pmg::MgmtInterface* mgmt_itf{param_handler.get()};
  mgmt_itf->set_value("state_machine.min_valid_imus", 1);
